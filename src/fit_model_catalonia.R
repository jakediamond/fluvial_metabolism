...
df_data <- read_fun(file.path("data", "catalonia", "catalonia_clean.xlsx")) |>
  filter(site == "REGAS") |>
  mutate(datestart = "2019-06-08", dateend = "2019-06-08") |> #3/10 before
  mutate(
...
# ini_data <- c(O2 = 0.2646, DIC = 1.82, ALK = 1.79)
...
parms_test_o2 <- c(K600 = 8, #O2_l = 0.2, qL = 4.6E-5,
                   gpp_mean = 1, er_mean = 4)
plower_o2 <- c(K600 = 4, #O2_l = 0.01, qL = 1E-5,
               gpp_mean = 0, er_mean = 0)
pupper_o2 <- c(K600 = 30, #O2_l = 0.2, qL = 1E-4,
               gpp_mean = 10, er_mean = 10)
...
var0 <- fito2$var_ms_unweighted
cov0 <- summary(fito2)$cov.scaled * 2.4^2/5

MCMC_o2 <- modMCMC(f = cost, p = fito2$par, upper = pupper_o2, lower = plower_o2)
                # niter = 5000, jump = cov0, var0 = var0, wvar0 = 0.1, updatecov = 50)
summary(MCMC_o2)
plot(MCMC_o2)
parms_fito2m <- replace(parms_data, names(MCMC_o2$bestpar), MCMC_o2$bestpar)
fitouto2m <- ode(i_dat_fito2, times, model, parms_fito2m,
                light_forcing = TRUE, temp_forcing = TRUE)
# parms_fito2
# out2 <- ode(ini_data, times, model_to_solve2, coef(fit))
plot(fitouto2m, obs=datcomp, obspar=list(pch=16, col="red"))
...
# sum(costs)
# sum((dat[-1] - out1[-1])^2)
parms_test <- c(K600 = 80, #DIC_l = 1.4, ALK_l = 0.4, O2_l = 0.2, qL = 4E-6,
                gpp_mean = 0.2, er_mean = 8, PQ = 0.95, RQ = 1.1)
plower    <- c(K600 = 20, #DIC_l = 0.1, ALK_l = 0.1, O2_l = 0, qL = 1E-7,
               gpp_mean = 0, er_mean = 1, PQ = 0.8, RQ = 0.8)
pupper    <- c(K600 = 200, #DIC_l = 2, ALK_l = 0.8, O2_l = 0.4, qL = 1E-5,
               gpp_mean = 5, er_mean = 15, PQ = 1.5, RQ = 1.5)
...
MCMC <- modMCMC(f = cost, p = parms_test, upper = pupper, lower = plower,
                niter = 5000)#, jump = 8,
...
plot(MCMC)
MCMC$bestpar
parms_fitm <- replace(parms_data, names(MCMC$bestpar), MCMC$bestpar)
i_dat_fitm <- c(ini_data, "CALC" = parms_data[["CaCO3"]])
fitoutm <- ode(i_dat_fitm, times, model, parms_fitm, 
               light_forcing = TRUE, temp_forcing = TRUE)
parms_fitm
# out2 <- ode(ini_data, times, model_to_solve2, coef(fit))
plot(fitoutm, obs=datcomp, obspar=list(pch=16, col="red"))
