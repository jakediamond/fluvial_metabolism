...
df_data <- read_fun(file.path("data", "florida", "florida_clean.xlsx")) |>
  filter(site == "SF2500") |>
  mutate(datestart = "2019-06-29", dateend = "2019-06-29") |>
  mutate(
...
parms_test_o2 <- c(K600 = 1, O2_l = 0.1, qL = 3E-4,
  gpp_mean = 5, er_mean = 12)
plower_o2 <- c(K600 = 0.5, O2_l = 0.01, qL = 1E-4,
                gpp_mean = 2, er_mean = 5)
pupper_o2 <- c(K600 = 5, O2_l = 0.2, qL = 8E-4,
               gpp_mean = 10, er_mean = 20)
...
# sum(costs)
# sum((dat[-1] - out1[-1])^2)
parms_test <- c(K600 = 2, DIC_l = 1, ALK_l = 1, O2_l = 0.1,
                gpp_mean = 5, er_mean = 5, PQ = 1, RQ = 1)
plower    <- c(K600 = 0.5, DIC_l = 0.5, ALK_l = 0.5, O2_l = 0,
               gpp_mean = 1, er_mean = 1, PQ = 0.8, RQ = 0.8)
pupper    <- c(K600 = 4, DIC_l = 3, ALK_l = 3, O2_l = 0.4,
               gpp_mean = 10, er_mean = 10, PQ = 1.5, RQ = 1.5)
...
# These are the results from a fit form MCMC from Stan
pfit_mcmc    <- c(K600 = 3.41/1.7, DIC_l = 0.9, ALK_l = 0.467, O2_l = 0.161, qL = 3.2E-5*50,
             gpp_mean = 9, er_mean = 12, PQ = 1, RQ = 1)
parms_fitmcmc <- replace(parms_data, names(pfit_mcmc), pfit_mcmc)
fitoutmcmc <- ode(c(O2 = 0.141, DIC = 0.831, ALK = 0.610, CALC = 0), times, model, parms_fitmcmc, 
               light_forcing = TRUE, temp_forcing = TRUE)
# parms_fitmcmc
# out2 <- ode(ini_data, times, model_to_solve2, coef(fit))
plot(fitoutmcmc, obs=datcomp, obspar=list(pch=16, col="red"))
